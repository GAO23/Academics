...
Test(sf_memsuite_student, realloc_same_size, .init = sf_mem_init, .fini = sf_mem_fini) {
    ...
    sf_block *bp = (sf_block *)((char*)y - 2*sizeof(sf_header));
    cr_assert((bp->header & BLOCK_SIZE_MASK) == 32, "block size wrong");
    cr_assert(x == y, "realloc does not return the same thing");
...
Test(sf_memsuite_student, malloc_free_malloc, .init = sf_mem_init, .fini = sf_mem_fini) {
    ...
    *((double*)x) = 100.0;
    ...
    cr_assert_not_null(x, "x should not be null");
    cr_assert( *((double*)x)  == 100.0, "x should be 100");

}

Test(sf_memsuite_student, malloc_free, .init = sf_mem_init, .fini = sf_mem_fini) {
    void *x = sf_malloc(4);
    sf_free(x);
    sf_block *bp = (sf_block *)((char *)x - 2*sizeof(sf_header));
    cr_assert((bp->header & 0b010) == 0, "block still allocated");
}

Test(sf_memsuite_student, malloc_malloc_free_realloc, .init = sf_mem_init, .fini = sf_mem_fini) {
    void *x = sf_malloc(4);
    void *y = sf_malloc(8);
    sf_free(x);
    sf_realloc(x, 16);
    sf_block *bp = (sf_block *)((char *)x - 2*sizeof(sf_header));
    cr_assert((bp->header & BLOCK_SIZE_MASK) == 32, "block size not 32");
    cr_assert_not_null(y, "y is null");
}
