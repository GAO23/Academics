...

{
  ...
  while (T) {
    ...
    switch (len = getline(&buffer_ptr, &size, iport)) {
      case (AT_EOF) :
        return(ANSWER_EOF);
        break;
      case (TOO_MANY_CHARS) :
...
int custom_getline (iport,buffer,buflen) FILE *iport; char *buffer; int buflen;
...

{
  int len;
  char *line;
  if (0 > (len = getline(&linebuf, (size_t*) &maxlinelen, fp))) {
     if (len == AT_EOF) {
...
{  
  ...
  while (T) {
    ...
    if (0 > (len = getline(&linebuffer, (size_t*) &linebuflen, fp))) {
       fclose(fp);
       if (len == AT_EOF) {
          return(linecount);
       }
