...
function init(passport, getUserByEmail, getUserById){
    ...
    passport.use(new LocalStrategy({usernameField: 'username'}, callback));
    passport.serializeUser((user, done) => done(null, user._id)); // user is user if return by authenticate_user and done is just like in the authenticate_user, this stores the user document object id in session
    passport.deserializeUser( async (id, done) =>{
        try{
            var user = await getUserById(id);
            done(null, user);
        }catch (e) {
            done("we fucked up", false);
        }
