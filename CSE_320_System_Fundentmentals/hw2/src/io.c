...

{
  ...
  //Ptr_Rolo_Entry oldentry, currententry;
  Ptr_Rolo_List newlink,rptr;
  char *next_field,*next_other;
//  char *oldname,*currentname;
  char **other_pointers;
  int n_entries = 0;
...

{
  ...
  Ptr_Rolo_List rptr = NULL;
  Ptr_Rolo_Entry entry = NULL;
  int j = 0;
...
int display_basic_field (char *name, char *value, int show, int up)
{
//  int semi = 0;
  int i;
  if (all_whitespace(value) && !show) return 0;
  printf("%-25s",name);
  while (*value != '\0') {
    if (*value == ';') {
       while (*++value == ' '); 
